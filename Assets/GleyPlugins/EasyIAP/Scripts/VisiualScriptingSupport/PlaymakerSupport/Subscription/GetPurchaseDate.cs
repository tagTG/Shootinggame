...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Get Subscription Purchase Date")]
    public class GetPurchaseDate : FsmStateAction
    {
        ...
        [Tooltip("Variable where the Purchase Date will be stored")]
        ...
        public override void OnEnter()
        {
            if (IAPManager.Instance.IsInitialized())
            {
                ...
                if (info != null)
                {
                    result.Value= info.getPurchaseDate().ToShortDateString();
                }
                else
                {
                    result.Value = "-";
