...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Get Subscription Introductory Price Period State")]
    public class IsIntroductoryPricePeriod : FsmStateAction
    {
        ...
        [Tooltip("Variable where the Introductory Price Period State will be stored")]
        ...
        public override void OnEnter()
        {
            if (IAPManager.Instance.IsInitialized())
            {
                ...
                if (info != null)
                {
                    result.Value = info.isIntroductoryPricePeriod().ToString();
                }
                else
                {
                    result.Value = "-";
